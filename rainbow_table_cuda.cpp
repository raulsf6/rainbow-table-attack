...
bool findPassInChain(string chain_start, string hash1){
	bool found = false;
	uint8_t* out = new uint8_t[32];
	char *base64 = new char[7];
...
int findHashInMap(vector<string> values, string hash){
	...
	for(int i = 0; i < values.size(); ++i){
		if (strncmp(array[i], hash.c_str(), hash.size()) == 0){
			return i;
		} 
	}
	//Not found
...
// CAN BE CUDA
bool searchHash(vector<string> keys, vector<string> values, string hash1){
	...
	uint8_t* out = new uint8_t[32];
	while(!found && i >= -1){
		index = findHashInMap(values, hash1);
		if (index != -1){
			chain_start = keys[index];
			if (findPassInChain(chain_start, original_hash) == true){
				found = true;
			}
			else
			{
